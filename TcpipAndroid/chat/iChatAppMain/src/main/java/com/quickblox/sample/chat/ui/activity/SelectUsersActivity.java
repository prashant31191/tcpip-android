...
public class SelectUsersActivity extends BaseActivity {
    ...
    private void loadUsersFromQb() {
        ...
        tags.add(Consts.QB_USERS_TAG2);
       //111 tags.add(Consts.QB_USERS_TAG2);
       //111 tags.add("android");
        tags.add("ios");
        tags.add("bbbbbbbbb");
        Logs.showLog("=========tags=======");

        //tags.add(Consts.QB_USERS_TAG2);
        //tags.add(Consts.QB_USERS_TAG3);

        progressBar.setVisibility(View.VISIBLE);
        countUser = 0;

        final ArrayList<QBUser> resultAll = new ArrayList<>();
        final ArrayList<QBChatDialog> arrQBChatDialog = new ArrayList<>();

        for (String tagSingle : tags) {
            Logs.showLog("=========tags======="+tagSingle);

            ArrayList<String> listTagSingle = new ArrayList<>();
            listTagSingle.add(tagSingle);

            QBUsers.getUsersByTags(listTagSingle, null).performAsync(new QBEntityCallback<ArrayList<QBUser>>() {
                @Override
                public void onSuccess(ArrayList<QBUser> result, Bundle params) {
                    Logs.showLog("=========tags====Add==result.size=" + result.size());
                    QBChatDialog dialogSingle = (QBChatDialog) getIntent().getSerializableExtra(EXTRA_QB_DIALOG);
                    resultAll.addAll(result);
                    if(dialogSingle !=null) {
                        arrQBChatDialog.add(dialogSingle);
                    }
                    countUser = countUser +1;
                    if (countUser == tags.size() )
                    {

                        Logs.showLog("========11111111111111111111111111=================");
                        usersAdapter = new CheckboxUsersAdapter(SelectUsersActivity.this, resultAll);

                        if (arrQBChatDialog != null && arrQBChatDialog.size() > 0) {
                            for (QBChatDialog qbChatDialog : arrQBChatDialog) {
                                Logs.showLog("=========tags====Add dialog==dialog.size=" + arrQBChatDialog.size());

                                if(qbChatDialog!=null && qbChatDialog.getOccupants() !=null) {
